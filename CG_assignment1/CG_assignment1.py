...
class Win(GlutWindow):
	...
	def calc_model(self):
		...
		# Calculating the rotation angle (degrees)
		rotation_angle = time_taken * 30  # Rotate 50 degrees per second
		# Rotating the model matrix (arund the x-axis)
		self.model_matrix = glm.rotate(self.model_matrix, glm.radians(rotation_angle), glm.vec3(1, 0, 0))
